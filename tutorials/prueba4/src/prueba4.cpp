...
Prueba4::Prueba4(Engine* t_engine) : engine(t_engine), pad(&t_engine->pad) {}
...
void Prueba4::loop() {
  ...
  if(pad->getLeftJoyPad().v <= 100){
    sprite.position -= Vec2(0,1); //up
  }else if(pad->getLeftJoyPad().v >= 200){
    sprite.position += Vec2(0,1); //down
  }

   if (pad->getLeftJoyPad().h <= 100) {
      sprite.position -= Vec2(1,0); // left
   } else if (pad->getLeftJoyPad().h >= 200) {
      sprite.position += Vec2(1,0); //right
   }
  ...
  /* PORQUE NO USAR, SE INTENTA MODIFICAR UNA TEXTURA CON SU 
     TEXTUREBUFFER YA CREADO, SE INTENTA BORRAR EN EL CASO DE QUE EXISTA
     ENTONCES EL PUNTERO DEL VRAM VA A LA POSICION DONDE ESTABA
     NO SE CUAL ES EXACTAMENTE EL PROBLEMA PERO EN UN MOMENTO DEJA DE FUNCIONAR
     :P
  */
  /*if(i==0){
    auto* texture = renderer.getTextureRepository().getBySpriteId(sprite.id);
    ...
    if(renderer.core.texture.getAllocatedBuffersByTextureId(texture->id).id != 0){
      renderer.core.gs.vram.free(renderer.core.texture.getAllocatedBuffersByTextureId(texture->id).core->address);
  ...
  /*packet2_t* packet = packet2_create(16, P2_TYPE_NORMAL, P2_MODE_NORMAL, 0);
  ...
  packet2_update(packet,draw_point(packet->next,0,point));
...
bool Prueba4::existFrameSprite(Sprite* sprite){
  bool exist = false;
  for(u32 i=0;i < id_spr.size() ;i++){
    if(sprite->id == id_spr[i]){ 
      ...
      existFrameTexture(sprite,&spr_effects[i]);
      return true;
    }
  }
  ...
  exist = true;
  existFrameTexture(sprite,&spr_effects[id_spr.size()-1]);
  //printf("id de la nueva textura:%d\n",spr.id);
  return exist;
}

bool Prueba4::existFrameTexture(Sprite* sprite,Sprite* spr){
  auto* texture = engine->renderer.core.texture.repository.getBySpriteId(sprite->id);
  // se encontro textura del sprite original
  //printf("se encontro la textura del sprite original\n");
  if(texture!=nullptr){
    auto* texSpr = engine->renderer.core.texture.repository.getBySpriteId(spr->id);

    // si se movio el sprite o se cambio el tamaño
    // se borra la textura
    if(spr->position.x != sprite->position.x || spr->position.y != sprite->position.y ||
       spr->size.x != (sprite->size.x*sprite->scale) || spr->size.y != (sprite->size.y*sprite->scale)){
        if(texSpr!=nullptr){
          engine->renderer.getTextureRepository().free(texSpr);
          texSpr = nullptr;
          //printf("se borro la textura\n");
        }
        //if(texSpr==nullptr){
        //  printf("texspr es nulo\n");
        //}else{printf("texspr DEBERIA SER nulo\n");}
        
        spr->position = sprite->position;
        spr->size.x = sprite->size.x * sprite->scale;
        spr->size.y = sprite->size.y * sprite->scale;
    }
    if(texSpr==nullptr){
      printf("no existe la textura para el sprite nuevo/modificado, se va a crear\n");
      PngLoader loader;
      unsigned char* pixel_texture = static_cast<unsigned char*>(memalign(
        128, loader.getTextureSize(spr->size.x+1, spr->size.y+1, bpp32))); 

      // obtengo los pixeles del framebuffer
      ps2_screenshot(pixel_fb,0,0,0,512,448,GS_PSM_32);
      struct PngPixel4* pngPixel= (struct PngPixel4*) pixel_fb;
      struct PngPixel4* pngPixelTexture= (struct PngPixel4*) pixel_texture;
      int posY = sprite->position.y;
      if(posY < 0){ posY = 0; }
      int initialPixel = (posY * 512) + sprite->position.x;
      int counterX=0;
      int counterY=0;
      int counter =0;
      int maxPixelX = spr->size.x+1;
      int maxPixelY = spr->size.y+1;
      printf("initialPos:%d\n",initialPixel);
      printf("maxPixels: %d,%d\n",maxPixelX,maxPixelY);
      //printf("size:%f,%f\n",spr->size.x,spr->size.y);
      //printf("scale:%f\n",spr->scale);
      for(int i=initialPixel;i<512*448;i++){
        if(counterX==maxPixelX){ i+=(512-maxPixelX); counterX=0;counterY++;}
        if(counterY==maxPixelY){ break;}
        if(counterY==10 && counterX%2==0){
          pngPixel[i].r = 255;
          pngPixel[i].g = 0;
          pngPixel[i].b = 0;
        }
        //printf("pixel[%d][%d]:%d: %d,%d,%d,%d\n",counterY,counterX,i,pngPixel[i].r,pngPixel[i].g,pngPixel[i].b,pngPixel[i].a);
      ...
      auto* data = texture->getData(); // obtengo los datos del sprite
      texSpr = new Texture(data); 
      delete data;
      texSpr->core->width  = spr->size.x+1;
      texSpr->core->height = spr->size.y+1;
      texSpr->core->data   = pixel_texture;
      engine->renderer.getTextureRepository().add(texSpr);
      texSpr->addLink(spr->id);
    ...
    printf("size:%f,%f\n",spr->size.x,spr->size.y);
    printf("scale:%f\n",spr->scale);
    engine->renderer.renderer2D.render(spr);
    return true;
  }
  return false;
}

void Prueba4::perfectPixelSize(Sprite* sprite){
...
int Prueba4::renderMask(Sprite* sprite, Sprite* mask){
